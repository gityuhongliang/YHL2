		...
		init:function(){
			...
			if(this.options.slide){//动画切换左右划入划出
				...
				this.$courselItems.on('move',function(ev){
					var index =_this.$courselItems.index(this) //获取点击下标用来if判断
					if (_this.now != index) {  //判断当前下标不等于
						_this.$elem.trigger('coursel-show',[index,this])
					}
				//自定义事件，传递信息，监听coursel-show
					
				})
				this._tab =this._slide; //划入划出
			}else{//动画切换淡入淡出
				//1.隐藏所有图片显示当前图片
				this.$elem.addClass('fade');
				this.$courselItems.eq(this.now).show();
				//4.事件代理监听点击左右按钮实现动画切换
				this._tab =this._fade;  //淡入淡出
				//5.初始化显示隐藏插件
